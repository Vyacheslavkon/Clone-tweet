...
async def test_post_tweet(
    ...
    query_tweet = select(models.Tweet).where(models.Tweet.id == response.json()["tweet_id"])
...
async def test_update_media_with_create_tweet(client: AsyncClient, test_session: AsyncSession,
    ...
    result = await test_session.execute(select(models.Media).where(models.Media.id == media.id))
