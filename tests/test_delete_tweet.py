...
async def test_delete_tweet(
    ...
    query_tweet = select(models.Tweet).where(models.Tweet.id == test_tweet_with_media.id)
    result = await test_session.execute(query_tweet)
    tweet = result.scalars().one_or_none()

    query_media = select(models.Media).where(models.Media.tweet_id == test_tweet_with_media.id)
