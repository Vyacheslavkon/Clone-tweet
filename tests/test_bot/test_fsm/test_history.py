...
async def test_report_history(
    test_dp, mock_bot, create_mock_update, test_user, test_i18n
):
...
async def test_history_two_week(
    test_dp,
    test_session,
    test_i18n,
    mock_bot,
    ...
    state = test_dp.fsm.get_context(
        bot=mock_bot, user_id=test_user.tg_id, chat_id=test_user.tg_id
    )
    ...
    with test_i18n.context():
        ...
        data_cur_week = await get_report_period(
            test_session, test_user.tg_id, start_cur_week, end_cur_week
        )

        data_last_week = await get_report_period(
            test_session, test_user.tg_id, start_prev_week, end_prev_week
        )
...
async def test_history_arbitrary_period(
    test_dp,
    test_session,
    mock_bot,
    create_mock_update,
    ...
    with test_i18n.context():
        ...
        update_step_1 = callback_message(
            data="period", user_id=test_user.tg_id, update_id=1
        )
        ...
        start_callback_data = SimpleCalendarCallback(
            act="DAY", year=2026, month=5, day=1
        )
        ...
        with patch.object(
            SimpleCalendar, "process_selection", new_callable=AsyncMock
        ) as mock_select:
        ...
        end_callback_data = SimpleCalendarCallback(
            act="DAY", year=2026, month=5, day=15
        )
        ...
        expected_start, expected_end = get_arbitrary_period(
            mock_start_date, mock_end_date
        )
        db_data = await get_report_period(
