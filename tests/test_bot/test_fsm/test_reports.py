...
async def test_report_period(
    test_dp, create_mock_update, mock_bot, test_i18n, test_user
):
...
async def test_report_periods(
    ...
    callback_message = create_callback(
        data=callback_data, user_id=test_user.tg_id, update_id=1
    )
...
async def test_report_month(
    ...
    callback_message = create_callback(
        data="month", user_id=test_user.tg_id, update_id=1
    )
