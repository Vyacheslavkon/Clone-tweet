...
def process_expense_task(self,chat_id: int, db_user_id: int, locale: str, voice_bytes: bytes):
    ...
    try:
        ...
        logger.warning(
            f"Сбой OpenAI API (ID запроса в логе выше). "
            f"Попытка ретрая {self.request.retries + 1}/3. Ошибка: {exc}"
        )
        ...
        logger.error(f"Критическая необрабатываемая ошибка в таске: {e}")
        raise e