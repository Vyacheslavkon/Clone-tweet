...
def get_test_analysis_financial(days: int = 30, actual_days: int = 30) -> str:
    ...
    if days == 7:
        instruction = (
            ...
            f"- Проанализируй текущую календарную неделю Анализируй строго прошедшие {actual_days} дн. с понедельника текущей недели!.\n"
            "- Сфокусируйся на импульсивных, незапланированных покупках, крупных разовых тратах и динамике доходов.\n"
            "- Оцени оперативный баланс недели 'net_balance'. Если неделя закрыта в плюс, отметь это.\n"
        ...
        instruction = (
            ...
            "- В 'top_items' тебе передан сырой хронологический поток транзакций (расходов). Самостоятельно сгруппируй семантически похожие названия трат "
    ...
    1. Поле 'top_items' содержит сырые  расходы, распознанные из сообщений. Названия могут слегка отличаться из-за специфики речи.
...
def get_tests_analysis_financial(period_type: int) -> str:
    if period_type == 7:
        instruction = (
...
def get_my_test_analysis_financial(days: int = 30, actual_days: int = 30) -> str:
    # Динамически вычисляем текущий день недели и дату для жесткого тайм-контекста
    ...
    if days == 7:
        instruction = (
            ...
            f"- Контекст: Сегодня {current_day_name}, {current_date}. Проанализируй строго прошедшие {actual_days} дней.\n"
        ...
        instruction = (
            ...
            f"- Контекст: Сегодня {current_date}. Текущий месяц еще не завершен. Анализируй строго прошедшие {actual_days} дней с 1-го числа текущего месяца!\n"
    ...
    ⚙️ РАБОТА С СЫРЫМИ ДАННЫМИ ТРАНЗАКЦИЙ (поле 'top_items'):
    1. Поле 'top_items' содержит сырые расходы, распознанные из сообщений (массив объектов с 'label', 'amount', 'date').
    2. Тебе РАЗРЕШЕНО и необходимо суммировать цены внутри 'top_items' для одинаковых или семантически близких товаров, чтобы рассчитать стоимость конкретной привычки для блока рекомендаций. Это не нарушает Табу №1.
    ...
    2. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО давать рекомендации по урезанию или экономии на расходах типа 'essential'. Если у пользователя высокие траты в этой зоне, прояви искреннюю эмпатию в общем резюме (summary), но не трогай их в советах.
    3. Строй блок рекомендаций (recommendations) исключительно на основе товаров и привычек, которые ты классифицировал как 'discretionary'.
    4. Каждая рекомендация должна быть жестко привязана к конкретным текстовым названиям ('label') из списка 'top_items'. Запрещено использовать абстрактные названия категорий в качестве заголовков рекомендаций.
