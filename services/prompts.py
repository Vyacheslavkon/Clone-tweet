...
def get_analysis_expense(days: int = 30, actual_days: int = 30) -> str:
    if days == 7:
        instruction = (
        ...
        instruction = (
            ...
            "- Вместо этого выяви скрытые системные привычки и повторяющиеся ритуалы на основе частоты покупок (поле 'count').\n"
            "- Математически подсвети накопительный эффект: если товар стоит мало, но куплен 15 раз, посчитай общую сумму этой привычки за месяц.\n"
            "- Тон: глубокий, чуткий, стратегический, как у личного финансового коуча."
        )

    system_prompt_analysis_expense = f"""
    Ты — эмпатичный, профессиональный финансовый аналитик и коуч. Твоя задача — провести глубокий аудит трат пользователя.
    ...
    1. Тебе ЗАПРЕЩЕНО самостоятельно рассчитывать, округлять или изменять общую сумму расходов ('Всего потрачено') и суммы категорий.
    2. Используй только те числовые значения, которые переданы в явном виде в 'user_context'. 
